  ...
  if (isDesktop) {
    ...
    const D7RightContent = () => {
      ...
      if (step === "Insight") return (
        <div key={idx} className="au-step-enter" style={{padding:"44px 52px",overflowY:"auto"}}>
          ...
          <h2 style={{fontFamily:T.serif,fontSize:40,fontWeight:600,color:T2.text,lineHeight:1.1,marginBottom:16}}>Most people know how to communicate better. Very few practise until it's automatic.</h2>
          <p style={{fontFamily:T.sans,fontSize:18,color:"#A8998A",lineHeight:1.6,fontWeight:400,marginBottom:32}}>This week, you did something different.</p>
      ...
      if (step === "Rehearsal") {
        return (
          <div key={idx} className="au-step-enter" style={{padding:"44px 52px",overflowY:"auto"}}>
            <D7PracticeWidget T={T} T2={T2} isDesktop={true} onSimulation={() => setIdx(STEPS.indexOf('Simulation'))} onNavLabel={setD7NavLabel} onNavFn={d7NavFnRef}/>
          </div>
        );
      }
    ...
    const D10RightContent = ({onRecordingChange}) => {
      ...
      if (step === "Insight") return (
        <div key={idx} className="au-step-enter" style={{padding:"44px 52px",overflowY:"auto"}}>
          ...
          <p style={{fontFamily:T.sans,fontSize:18,color:"#A8998A",lineHeight:1.6,fontWeight:400,marginBottom:32}}>In 1996, Harvey Coleman found that Performance accounts for just 10% of career advancement. Not because it doesn't matter — but because it is not the multiplier.</p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:28}}>
            {D10_FACTS.map((n,i)=>{
      ...
      if (step === "Example") {
        const D10_EDITORIAL = [
          ...
          { id:"bezos", img:"/d10-bezos.jpg", imgPos:"center 50%", name:"Jeff Bezos", role:"Founder of Amazon", superpower:"Master of Repeatable Excellence",
            ...
            summary:"He built one of the world's largest companies by focusing relentlessly on systems — so that great performance happened at scale, not by accident.",
            exploreLabel:"Explore his techniques",
            body1:"Jeff Bezos built one of the world's largest companies by focusing relentlessly on systems rather than individual moments of brilliance. From warehouses to customer service, every process was designed to deliver the same high standard, every single time.",
                ...
                return (
                  <div key={card.id} onClick={()=>openReading(card.id)}
                    ...
                    <div style={{height:240,overflow:"hidden",position:"relative"}}>
                      <img loading="lazy" src={card.img} alt={card.name} style={{width:"100%",height:"100%",objectFit:"cover",objectPosition:"center center",display:"block",transition:"transform 0.35s ease, opacity 0.4s ease",opacity:0}} onLoad={e=>e.currentTarget.style.opacity="1"}/>
            ...
            {sarResult && (
              <div className="au-step-enter" style={{background:T2.surface,borderRadius:4,border:"0.5px solid "+T2.border,padding:"24px 28px",marginBottom:16}}>
                ...
                <div style={{marginTop:16,borderLeft:"3px solid "+T.gold,background:"rgba(138,158,132,0.05)",borderRadius:"0 4px 4px 0",padding:"14px 16px"}}>
                  <p style={{fontFamily:T.serif,fontSize:15,fontStyle:"italic",color:T2.text,lineHeight:1.6,margin:0}}>When someone asks what you've achieved, you'll have a story ready.</p>
                </div>
    ...
    const D3RightContent = () => {
      ...
      async function checkPace() {
        ...
        try {
          const res = await fetch("/api/claude",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-5",max_tokens:250,messages:[{role:"user",content:`Rewrite this as if spoken 20% slower — add natural pause markers [...] and break long sentences. Keep the same meaning. Never use em dashes; use a comma or hyphen instead. Return ONLY the rewritten version:\n\n"${paceInput}"`}]})});
    ...
    const D4RightContent = () => {
            ...
            {D4_FACTS.map((n,i)=>{
              ...
              return (
                <div key={i} onClick={()=>setD4CardOpen(open?null:i)}
                  ...
                  <p style={{fontFamily:T.sans,fontSize:14,color:T2.text3,lineHeight:1.5,fontWeight:400,margin:open?"0 0 14px":0}}>{n.sub}</p>
                  {open && (<div style={{borderTop:"0.5px solid "+T2.divider,paddingTop:14,display:"flex",flexDirection:"column",gap:8}}>
                    {n.bullets.map((b,j)=>(<div key={j} style={{display:"flex",gap:8,alignItems:"flex-start"}}><div style={{width:4,height:4,borderRadius:"50%",background:T.gold,flexShrink:0,marginTop:6}}/><p style={{fontFamily:T.sans,fontSize:14,color:T2.text,lineHeight:1.65,fontWeight:400,margin:0}}>{b}</p></div>))}
                  </div>)}
                </div>
              );
            })}
      ...
      if (step === "Example") {
        const D4_EDITORIAL = [
          ...
          { id:"chanel", img:"/d4-chanel.jpg", imgPos:"center 55%", name:"Coco Chanel", role:"Fashion designer & founder of Chanel", superpower:"Elegance Through Elimination",
            ...
            body2:"That same instinct for elimination shaped her design philosophy for decades — stripping away ornament, structure, and excess until only what mattered remained. She talked about clothing the same way she made it.",
      ...
      if (step === "Rehearsal") return (
        <div key={idx} className="au-step-enter" style={{padding:"44px 52px",overflowY:"auto"}}>
          <D4PracticeWidget T={T} T2={T2} isDesktop={true} onSimulation={() => setIdx(STEPS.indexOf('Simulation'))} onNavLabel={setD4NavLabel} onNavFn={d4NavFnRef}/>
        </div>
      );
      ...
      if (step === "Theory") return (
        <div key={idx} className="au-step-enter" style={{ padding:"44px 52px", overflowY:"auto" }}>
          ...
          <div style={{ padding:"20px 24px", background:T2.surface, borderRadius:4, borderLeft:"2px solid "+T.gold, marginBottom:28 }}>
            <p style={{ fontFamily:T.serif, fontSize:22, fontWeight:600, color:T2.text, lineHeight:1.4, margin:0, fontStyle:"italic" }}>The brain encodes information more deeply when processed through both verbal and visual channels simultaneously. Story activates both. Facts alone activate only one.</p>
          </div>
    ...
    const D9RightContent = () => {
      ...
      if (step === "Insight") return (
        <div key={idx} className="au-step-enter" style={{ padding:"44px 52px", overflowY:"auto" }}>
          ...
          <p style={{ fontFamily:T.sans, fontSize:18, color:"#A8998A", lineHeight:1.6, fontWeight:400, marginBottom:28, maxWidth:600 }}>Most people focus on what they want to say. Great communicators focus on understanding the person in front of them.</p>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:28 }}>
    ...
    const D2RightContent = () => {
      ...
      if (step === "Simulation") return (
        <div key={idx} className="au-step-enter" style={{ padding:"44px 52px", overflowY:"auto" }}>
          ...
          <D2SimWidget T={T} T2={T2} isDesktop={true} onRecordingChange={setD2SimRecording}/>
        </div>
      );
    ...
    const D5RightContent = () => {
      ...
      if (step === "Insight") return (
        <div key={idx} className="au-step-enter" style={{ padding:"44px 52px", overflowY:"auto" }}>
          ...
          <p style={{ fontFamily:T.sans, fontSize:18, color:T2.text3, lineHeight:1.6, fontWeight:400, margin:"0 0 32px" }}>Great communicators don't just have better ideas. They organise them better. Structure helps people understand faster, remember longer, and act with confidence.</p>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:28 }}>
            {[
      ...
      if (step === "Rehearsal") return (
        <div key={idx} className="au-step-enter" style={{ padding:"44px 52px", overflowY:"auto" }}>
          <D5PracticeWidget T={T} T2={T2} isDesktop={true} onSimulation={() => setIdx(STEPS.indexOf('Simulation'))} onRecordingChange={setD5RehearsalRecording}/>
        </div>
      );

      if (step === "Simulation") return (
        <div key={idx} className="au-step-enter" style={{ padding:"44px 52px", overflowY:"auto" }}>
          <D5SimWidget T={T} T2={T2} isDesktop={true} onRecordingChange={setD5SimRecording}/>
        </div>
      );
      ...
      if (step === "Example") {
        ...
        if (reading) return (
          <div key={"d5read"+reading.id} className="au-step-enter" style={{padding:"44px 52px",overflowY:"auto",maxWidth:680}}>
            ...
            <p style={{fontFamily:T.sans,fontSize:15,color:T2.text,lineHeight:1.8,fontWeight:300,margin:"0 0 14px"}}>{reading.body1}</p>
            {reading.body2Lines
              ? <div style={{display:"flex",flexDirection:"column",gap:14,margin:"0 0 14px"}}>
                  {reading.body2Lines.map((ln,li)=>(
                    <div key={li} style={{paddingLeft:14,borderLeft:"2px solid rgba(138,158,132,0.35)"}}>
                      <div style={{fontFamily:T.sans,fontSize:10,fontWeight:700,color:T.gold,textTransform:"uppercase",letterSpacing:"1.8px",marginBottom:4}}>{ln.label}</div>
                      <p style={{fontFamily:T.sans,fontSize:15,color:T2.text,lineHeight:1.7,fontWeight:300,margin:0}}>{ln.text}</p>
                    </div>
                  ))}
                </div>
              : <p style={{fontFamily:T.sans,fontSize:15,color:T2.text,lineHeight:1.8,fontWeight:300,margin:"0 0 14px"}}>{reading.body2}</p>
    ...
    const D6RightContent = () => {
      ...
      if (step === "Rehearsal") return (
        <div key={idx} className="au-step-enter" style={{ padding:"44px 52px", overflowY:"auto" }}>
          <D6PracticeWidget T={T} T2={T2} isDesktop={true} onSimulation={() => setIdx(STEPS.indexOf('Simulation'))} onRecordingChange={setD6RehearsalRecording}/>
        </div>
      );

      if (step === "Simulation") return (
        <div key={idx} className="au-step-enter" style={{ padding:"44px 52px", overflowY:"auto" }}>
          <D6SimWidget T={T} T2={T2} isDesktop={true} onRecordingChange={setD6SimRecording}/>
        </div>
      );
    ...
    const D12RightContent = () => {
      ...
      if (step === "Insight") return (
        <div key={idx} className="au-step-enter" style={{ padding:"44px 52px", overflowY:"auto" }}>
          ...
          <p style={{ fontFamily:T.sans, fontSize:18, color:"#A8998A", lineHeight:1.6, fontWeight:400, marginBottom:32, maxWidth:600 }}>Body language shapes how communication feels — helping people feel more connected, engaged, and at ease.</p>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:28 }}>
            {D12_FACTS.map((n,i)=>{
      ...
      if (step === "Theory") return (
        <div key={idx} className="au-step-enter" style={{ padding:"44px 52px", overflowY:"auto" }}>
          ...
          <div style={{ padding:"20px 24px", background:T2.surface, borderRadius:4, borderLeft:"2px solid "+T.gold, marginBottom:28 }}>
            <p style={{ fontFamily:T.serif, fontSize:22, fontStyle:"italic", color:T2.text, lineHeight:1.65, margin:0 }}>"When communication feels safe, the brain becomes more open to the message."</p>
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:28 }}>
            {[
      ...
      if (step === "Example") {
        const D12_EDITORIAL = [
          { id:"kennedy", img:"/d12-kennedy.jpg", imgPos:"center 35%", name:"Kennedy vs Nixon", role:"1960 US Presidential Debate", superpower:"Master of Executive Presence",
            ...
            body1:"Kennedy appeared calm, composed and confident. He looked directly into the camera, maintained steady eye contact, stood comfortably and projected quiet authority. Nixon, recovering from illness, appeared tense, perspired under the studio lights and avoided looking directly at viewers.",
            body2:"Radio listeners could only judge the words being spoken, and many leaned toward Nixon's grasp of policy detail. Television viewers could also see the composure gap — and that visual impression outweighed the argument itself.",
    ...
    const D13RightContent = () => {
      ...
      if (step==="Insight") return (
        <div key={idx} className="au-step-enter" style={{padding:"44px 52px",overflowY:"auto"}}>
          ...
          <p style={{fontFamily:T.sans,fontSize:18,color:"#A8998A",lineHeight:1.6,fontWeight:400,marginBottom:28,maxWidth:600}}>The most talented professionals in every field are not always the most successful. The ones who advance are the ones whose value is seen.</p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:28}}>
      ...
      if (step==="Example") {
        const D13_EDITORIAL = [
          { id:"obama", img:"/d13-obama.jpg", imgPos:"center 50%", name:"Michelle Obama", role:"Former First Lady of the United States", superpower:"Master of Warm Presence",
            ...
            whyItWorks:"Warmth builds trust faster than authority does. When someone feels safe enough to be honest about their own uncertainty, it gives everyone else permission to relax — and people open up, remember more, and connect harder with speakers who feel human rather than flawless.",
            technique:"Before your next high-stakes conversation, ask: \"Am I trying to look confident, or trying to connect?\" Drop the armour. A moment of real honesty — even something small — does more for trust than a flawless delivery ever will.",
            lesson:"Presence isn't about filling a room with your voice — it's about making the room feel safe enough for others to bring theirs. The most memorable communicators aren't the most polished. They're the most human.",
          },
      ...
      if (step==="Rehearsal") return (
        <div key={idx} className="au-step-enter" style={{padding:"44px 52px",overflowY:"auto"}}>
          <h2 style={{fontFamily:T.serif,fontSize:40,fontWeight:600,color:T2.text,lineHeight:1.1,marginBottom:16}}>Make Yourself Visible</h2>
          <div style={{display:'inline-flex',alignItems:'center',gap:8,background:'rgba(138,158,132,0.08)',borderRadius:20,padding:'7px 14px',border:'0.5px solid rgba(138,158,132,0.22)',marginBottom:24}}>
            <svg width="15" height="15" viewBox="0 0 20 20" fill="none">
              <circle cx="10" cy="10" r="8.5" stroke="rgba(138,158,132,0.8)" strokeWidth="1.5"/>
              <path d="M10 6v4l2.5 2" stroke="rgba(138,158,132,0.8)" strokeWidth="1.5" strokeLinecap="round"/>
    ...
    const D14RightContent = () => {
      ...
      if (step==="Insight") return (
        <div key={idx} className="au-step-enter" style={{padding:"44px 52px",overflowY:"auto"}}>
          ...
          <p style={{fontFamily:T.sans,fontSize:18,color:"#A8998A",lineHeight:1.7,fontWeight:400,marginBottom:8,maxWidth:640}}>Over the last 14 days, you've developed skills that many people spend years trying to master.</p>
      ...
      if (step==="Theory") return (
        <div key={idx} className="au-step-enter" style={{padding:"44px 52px",overflowY:"auto"}}>
          ...
          <div style={{padding:"24px",background:T2.surface,borderRadius:4,border:"0.5px solid "+T2.border,marginBottom:20}}>
            ...
            <D14LoopDiagram T={T} T2={T2} isDesktop={true}/>
          ...
          {!d14Revealed ? (
            <>
                ...
                {D14_SUPERPOWERS.map((sp)=>{
                  ...
                  return (
                    <div key={sp.id} onClick={()=>d14Toggle(sp.id)}
                      style={{position:"relative",padding:"22px 18px",background:isSel?"rgba(200,164,106,0.08)":T2.surface,borderRadius:8,border:`1px solid ${isSel?T.gold:T2.border}`,boxShadow:isSel?"0 2px 10px rgba(200,164,106,0.18)":"0 1px 3px rgba(20,16,10,0.04)",display:"flex",flexDirection:"column",alignItems:"center",textAlign:"center",cursor:"pointer",transition:"all 0.18s ease"}}>
                      {isSel && (
                        <div style={{position:"absolute",top:-7,right:-7,width:20,height:20,borderRadius:"50%",background:T.gold,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 1px 4px rgba(0,0,0,0.2)"}}>
                          <svg width="11" height="11" viewBox="0 0 12 12" fill="none"><path d="M2.5 6.2l2.3 2.3 4.7-5" stroke="#fff" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/></svg>
                        </div>
                      )}
                      <div style={{width:38,height:38,borderRadius:"50%",background:isSel?"rgba(200,164,106,0.16)":"rgba(138,158,132,0.10)",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:14}}>
                        {sp.icon}
                      </div>
              ...
              <div style={{padding:"26px 28px",background:"rgba(138,158,132,0.06)",borderRadius:8,border:"0.5px solid rgba(138,158,132,0.2)",display:"flex",alignItems:"center",gap:20}}>
                ...
                <div style={{flex:1}}>
                  <div style={{fontFamily:T.sans,fontSize:10,fontWeight:700,color:T.gold,textTransform:"uppercase",letterSpacing:"1.8px",marginBottom:6}}>{d14Selected.length>0?`You've selected ${d14Selected.length} superpower${d14Selected.length>1?"s":""}`:"Now It's Your Turn"}</div>
                ...
                {d14Selected.length>0 ? (
                  <button onClick={()=>setD14Revealed(true)} style={{display:"flex",alignItems:"center",gap:8,flexShrink:0,background:"transparent",border:"none",cursor:"pointer",padding:0}}>
                    <span style={{fontFamily:T.sans,fontSize:13,fontWeight:700,color:T.gold,whiteSpace:"nowrap"}}>Reveal my superpower</span>
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M4 10h11M11 5l5 5-5 5" stroke={T.gold} strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/></svg>
                  </button>
                ) : (
                  <svg width="20" height="20" viewBox="0 0 20 20" fill="none" style={{flexShrink:0,opacity:0.3}}><path d="M4 10h11M11 5l5 5-5 5" stroke={T2.text3} strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/></svg>
                )}
              </div>
            </>
          ) : (
            <div className="au-step-enter" style={{animation:"fadeUp 0.5s ease both"}}>
              <div style={{fontFamily:T.sans,fontSize:11,fontWeight:700,color:T.gold,textTransform:"uppercase",letterSpacing:"2px",marginBottom:16}}>Your Superpower</div>
              <div style={{fontFamily:T.serif,fontSize:22,fontWeight:600,color:T2.text,marginBottom:24}}>{selectedTiles.map(t=>t.label).join(" · ")}</div>
              <div style={{display:"flex",flexWrap:"wrap",gap:10,marginBottom:28}}>
                {selectedTiles.map(sp=>(
                  <div key={sp.id} style={{display:"flex",alignItems:"center",gap:8,padding:"10px 16px",background:"rgba(200,164,106,0.08)",border:"1px solid "+T.gold,borderRadius:24}}>
                    <div style={{width:22,height:22,borderRadius:"50%",background:"rgba(200,164,106,0.16)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                      {sp.icon}
                    </div>
                    <span style={{fontFamily:T.sans,fontSize:12,fontWeight:700,color:T2.text,textTransform:"uppercase",letterSpacing:"1px"}}>{sp.label}</span>
                  </div>
                ))}
              </div>
              ...
              <button onClick={()=>setD14Revealed(false)} style={{display:"flex",alignItems:"center",gap:6,background:"transparent",border:"none",cursor:"pointer",padding:0}}>
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M9 2L4 7l5 5" stroke={T2.text3} strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/></svg>
                <span style={{fontFamily:T.sans,fontSize:13,color:T2.text3,fontWeight:400}}>Choose different superpowers</span>
              </button>
            </div>
          )}
        </div>
          ...
          <div key="review-luxury" className="au-step-enter" style={{ flex: 1, display: "flex", overflow: "hidden" }}>
            ...
            <div style={{ width: "36%", flexShrink: 0, position: "relative", overflow: "hidden", display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
              ...
              <div style={{ position: "relative", zIndex: 2, padding: "40px 40px 44px", animation: "fadeUp 0.6s ease both" }}>
                <div style={{ fontSize: 11, letterSpacing: "0.2em", textTransform: "uppercase", color: T.gold, fontFamily: T.sans, fontWeight: 500, marginBottom: 20 }}>Day {lesson.day} · {lesson.tag}</div>
                <p style={{ fontFamily: T.serif, fontWeight: 600, color: "white", fontSize: "clamp(24px,2vw,32px)", lineHeight: 1.2, letterSpacing: "-0.3px", maxWidth: 320 }}>{lesson.title}</p>
              </div>
            </div>
                      ...
                      {isNT && (() => { try { return JSON.parse(localStorage.getItem("au1_stories") || "[]").length > 0; } catch { return false; } })() && (
                        <div style={{ display: "flex", alignItems: "flex-start", gap: 14, marginBottom: 32, padding: "16px 20px", background: T2.surface, borderRadius: 6, border: "0.5px solid " + T2.border }}>
                          <div style={{ width: 24, height: 24, borderRadius: "50%", background: "rgba(138,158,132,0.12)", border: "1px solid rgba(138,158,132,0.3)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 1 }}>
                            <svg width="10" height="10" viewBox="0 0 22 22" fill="none"><path d="M11 3v3M11 16v3M3 11h3M16 11h3M5.6 5.6l2.1 2.1M14.3 14.3l2.1 2.1M5.6 16.4l2.1-2.1M14.3 7.7l2.1-2.1" stroke={T.gold} strokeWidth="1.3" strokeLinecap="round"/></svg>
                          </div>
                          <span style={{ fontFamily: T.sans, fontSize: 15, color: T2.text, lineHeight: 1.65, fontWeight: 300 }}>Your generated stories are saved — find them anytime on the Story Lab brief screen, under "My Stories."</span>
                        </div>
                      )}
              ...
              <button
                onClick={() => {
                  if (isD1 && step === "Rehearsal" && d1NavFnRef.current) {
                    d1NavFnRef.current();
                  } else if (isD4 && step === "Rehearsal" && d4NavFnRef.current) {
                    d4NavFnRef.current();
