  ...
  if (isDesktop) {
    return (
      <div style={{ height: "100vh", display: "flex", fontFamily: T.sans, background: T.bg, overflow: "hidden" }}>
        <style>{`
          @keyframes ctaGlow {
            0%, 100% { box-shadow: 0 4px 18px rgba(0,0,0,0.22), 0 0 18px rgba(196,176,154,0.35), 0 0 40px rgba(196,176,154,0.15); transform: translateY(0); }
            50%       { box-shadow: 0 6px 28px rgba(0,0,0,0.28), 0 0 32px rgba(196,176,154,0.6),  0 0 64px rgba(196,176,154,0.28); transform: translateY(-3px); }
          }
        ...
        {/* Floating CTA — fixed bottom right, glowing */}
        <button onClick={onContinue} className="au-cta" style={{ position: "fixed", bottom: 32, right: 32, zIndex: 200, display: "flex", alignItems: "center", gap: 10, padding: "15px 36px", borderRadius: 10, border: "1px solid #C8A87E", background: "linear-gradient(135deg, #D4C0A6 0%, #C4B09A 50%, #B89D84 100%)", color: "#1a1510", fontSize: 15, fontWeight: 600, cursor: "pointer", fontFamily: T.sans, letterSpacing: "0.3px", animation: "ctaGlow 2.6s ease-in-out infinite", whiteSpace: "nowrap" }}>
