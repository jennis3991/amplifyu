...
export function D9PracticeWidget({T, T2, isDesktop}) {
  const TIPS = [
    {
      label:"Upgrade Your Opening Question",
      title:'Ask curiosity, not category.',
      body:'Instead of "What do you do?" try: "What\'s the most interesting thing you\'re working on right now?" Leading with curiosity — not category — invites a real answer and signals that you see the person, not the role.',
    },
    {
      label:"The 70/30 Rule",
      title:"Listen 70%. Speak 30%.",
      body:"In your next conversation, consciously hold back. Ask a question, then let the other person fill the space. The most memorable communicators don't dominate — they make others feel genuinely heard.",
    },
    {
      label:"Empathy in Practice",
      title:"Understand someone you disagree with.",
      body:"Before you respond to a differing view, pause to ask: what matters to this person, what pressures might they be under, and what might I be missing? Understanding first doesn't mean agreeing — it means connecting.",
    },
    {
      label:"Know Their Style",
      title:"Adapt to the person in front of you.",
      body:"People tend to communicate as one of four styles: the Driver (results-focused), the Thinker (detail-focused), the Connector (relationship-focused), or the Visionary (ideas-focused). The best communicators notice which style they're working with — and flex to meet it.",
    },
    {
      label:"Ask One Better Question",
      title:"Open doors others leave closed.",
      body:'"What\'s the biggest challenge you\'re facing right now?" or "What\'s something you\'re genuinely excited about?" These questions create real conversations. They show interest, invite depth, and build trust — which "How are you?" rarely does.',
    },
  ];
    ...
    <div style={{display:"flex",flexDirection:"column",gap:isDesktop?16:14}}>
      <div>
        ...
        <h2 style={{fontFamily:T.serif,fontSize:isDesktop?40:28,fontWeight:600,color:T2.text,lineHeight:1.1,margin:"0 0 16px"}}>
          Build the habits of the world's most connected communicators.
        </h2>
        <div style={{background:"rgba(138,158,132,0.07)",borderRadius:6,border:"0.5px solid rgba(138,158,132,0.18)",padding:"14px 16px",marginBottom:20}}>
          ...
          <p style={{fontFamily:T.serif,fontSize:isDesktop?15:14,fontStyle:"italic",color:T2.text2,lineHeight:1.65,margin:0}}>
            Connection isn't a gift. It's a choice you make in every conversation.
          </p>
        </div>
      ...
      {TIPS.map((tip,i)=>(
        <div key={i} style={cs.card}>
          <div style={cs.label}>{tip.label}</div>
          <p style={{fontFamily:T.serif,fontSize:isDesktop?19:16,fontWeight:600,color:T2.text,lineHeight:1.3,margin:"0 0 10px"}}>{tip.title}</p>
          <p style={{fontFamily:T.sans,fontSize:isDesktop?14:13,color:T2.text3,lineHeight:1.7,margin:0}}>{tip.body}</p>
        </div>
      ))}
    </div>
  );
