...
export function D1SimWidget({T, T2, isDesktop, warmUpTopic, onRecordingChange}) {
  ...
  function loadVoiceResult(r) {
    ...
    setRound1(null);
    audioRef.current=null; setPlaying(false); setAudioProgress(0); setAudioBroken(false);
  ...
  if(phase==='feedback' && feedback) {
    ...
    const TYPE_COLOR={filler:"#C4714A",ramble:"#B05C4A",strong:"#527060",unclear:"#B05C4A",clean:"#527060"};
    // If markers are missing (malformed/incomplete API response), show no
    // markers rather than a fixed generic template that would be
    // indistinguishable from genuine transcript-derived ones.
    const rawMarkers=feedback.markers||[];
    ...
    <div style={{display:"flex",flexDirection:"column",gap:isDesktop?14:12}}>
      ...
      {/* 5 STRENGTHS + OPPORTUNITY — personalised (skipped entirely for a
          non-genuine attempt, since feedback.worked is deliberately empty
          then — there's nothing real to praise or critique) */}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {feedback.wordUpgrades.map((u,i)=>(
              <div key={i} style={{display:"flex",flexDirection:"column",gap:5}}>
                ...
                {u.why&&<p style={{fontFamily:T.serif,fontSize:isDesktop?11.5:11,fontStyle:"italic",color:T2.text3,margin:"2px 0 0",lineHeight:1.5}}>Why it works: {u.why}</p>}
              </div>
            ))}
          </div>
        </div>
