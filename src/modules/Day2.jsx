...
export function D2SimWidget({T, T2, isDesktop, onRecordingChange}) {
  ...
  function doStop(){
    ...
    stopRecording((text, failed) => {
      ...
      // real signal data whenever m alone was null.
      const merged = (m || signal) ? { ...(m || {}), ...(signal || {}) } : null;
  ...
  async function analyzeText(text, metrics){
    ...
    // for that dimension (short recording, muted mic, etc.) the AI is asked
    // to judge it from the transcript instead of the prompt silently
